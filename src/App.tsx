...
export default function App() {
  ...
  const startMode = (mode: GameMode) => {
    setState(prev => ({ ...prev, selectedMode: mode, screen: 'PLAYING' }));
    setCurrentRound(0);
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (isCorrect) {
      ...
      setFeedback({ type: 'success', message: 'CORRECT! +10 PTS' });
      playSound('success');
      setTimeout(() => {
        if (currentRound < 9) {
  ...
  const renderPlaying = () => {
    ...
    switch(state.selectedMode) {
      ...
      case 'TAP_MODE': return <TapModeView 
        round={currentRound} 
        onNext={() => {
          addPoints(5); // 5 points per round for recall
...
const TimeTrialView = ({ round, question, timeLeft, onAnswer, feedback, isGameOver, onHome }: any) => {
  const q = question;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {q.options?.map((opt: string) => (
            <button
              ...
              className="p-4 border-2 border-y3k-purple rounded-lg font-pixel text-sm text-left hover:bg-y3k-purple/20 transition-colors flex justify-between items-center group"
            >
              {opt}
...
const TapModeView = ({ round, onNext, onHome, isGameOver }: any) => {
  const r = TAPPLE_ROUNDS[round];
...
const GlitchModeView = ({ round, glitch, onSolve, onHome, isGameOver }: any) => {
  const r = glitch;
...
const QuestModeView = ({ round, quest, onAnswer, feedback, isGameOver, onHome }: any) => {
  const q = quest;
        ...
        <div className="grid grid-cols-1 gap-4">
          {q.options.map((opt: any) => (
            <button
              ...
              className="p-6 border-2 border-y3k-silver rounded-lg font-pixel text-sm text-left hover:bg-y3k-silver/20 transition-colors flex justify-between items-center group"
            >
